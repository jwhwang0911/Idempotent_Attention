...
class DiscriminatorVGG128(nn.Module):
    def __init__(self, in_nc, base_nf, norm_type="batch", act_type="leakyrelu"):
        ...
        # 128, 64
        conv0 = conv_block(
            in_nc, base_nf, kernel_size=3, norm_type=None, act_type=act_type, padding=1
        )
        conv1 = conv_block(
            base_nf,
            base_nf,
...
class AFGSA(nn.Module):
    ...
    def forward(self, noisy, aux):
        ...
        k = rearrange(
            k, "b (k1 k2) (h d) -> (b h) k1 k2 d", k1=block + 2 * halo, h=heads
        )

        # print(f"q.shape = {q.shape}, k.shape = {k.shape}, v.shape = {v.shape}")

        # print(f"rel_h : {self.rel_h.shape}, rel_w : {self.rel_w.shape}")
...
class TransformerBlock(nn.Module):
    def __init__(self, ch, block_size=8, halo_size=3, num_heads=4, checkpoint=True):
        ...
        self.attention = AFGSA(
            ch, block_size=block_size, halo_size=halo_size, num_heads=num_heads
        )
