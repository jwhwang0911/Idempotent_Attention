...
def hdr_to_ldr(data: np.array):
    t_data = data.copy()
    t_data = np.clip(exr.tonemap(t_data) * 255.0, 0, 255.0).astype(np.uint8)
    return t_data
...
def inf_nan_to_num(value: np.ndarray):
    value = np.clip(value, 0.0, np.max(value))
...
def preprocess_data(exr_path, gt_path):
    ...
    curr_exr_path = exr_path.split(".exr")[0]
    curr_gt_path = gt_path.split(".exr")[0]

    # high spp
    types = [".exr"]
    names = ["gt"]
    for type, name in zip(types, names):
        path = curr_gt_path + type
        tmp_exr = exr.open(path)
        d = tmp_exr.get_all()
        data[name] = d["default"]
    # low spp
    types = ["_normal.exr", "_depth.exr", "_albedo.exr", ".exr"]
    names = ["normal", "depth", "albedo", "noisy"]
    for type, name in zip(types, names):
        path = curr_exr_path + type
        noisy_exr = exr.open(path)
        d = noisy_exr.get_all()
        if name == "depth":
            data[name] = d["default"][:, :, 0:1]
        else:
            data[name] = d["default"][:, :, 0:3]

    # nan to 0.0, inf to finite number
    for channel_name, channel_value in data.items():
        data[channel_name] = inf_nan_to_num(channel_value)
    ...
    data["gt"] = Image_Pre(data["gt"].copy())
    data["noisy"] = Image_Pre(data["noisy"].copy())

    # normalize auxiliary features to [0.0, 1.0]
    # data['normal'] = preprocess_normal(data['normal'].copy())
    data["depth"] = preprocess_depth(data["depth"])
...
def ExpTransform(data):

    return np.exp(data) - 1.0


def Image_Pre(val):
    return LogTransform(val.copy())


def Image_Post(val):
    return ExpTransform(val.copy())
...
def importance_sampling(data, patch_size, num_patches):
    ...
    patches = sample_patches_dart_throwing(
        buffers[0].shape[:2], patch_size, num_patches
    )
    ...
    pruned = np.maximum(
        0,
        prune_patches(buffers[0].shape[:2], patches + pad, patch_size, importance_map)
        - pad,
    )
...
def crop(data, position, patch_size):
    ...
    rx, ry = (px-hx) % tile_size, (py-hx) % tile_size
    px -= rx
    py -= ry
    ...
    for key, value in data.items():
        if key in ["albedo", "depth", "normal"]:
            ...
            assert (py - hy) % tile_size == 0 and (px - hx) % tile_size == 0, f"(py - hy) = {py - hy}, (px - hx) = {(px - hx)}  should be divided by Tilesize : {(tile_size)}"
