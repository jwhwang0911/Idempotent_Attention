...
def exr_to_png(file: Union[str, np.ndarray], distpath : str, channel:str='default', toneMapping: bool = True) -> np.ndarray:
  # For tonemapping
  gamma = 2.2
  
  if not isinstance(distpath, str) or not (distpath.endswith('.jpg') or distpath.endswith('.png')):
    raise Exception("File '%s' is not correct path" % distpath)
    
  
  if isinstance(file, np.ndarray):
    pass
  elif isinstance(file, str):
    file = read(file, channel)
  else:
    raise Exception("File format is invalid to store to jpg or png")
  
  value = np.array(file)
  if value.shape[2] > 4:
    raise Exception("Numpy format is invalid to store")
  elif value.shape[2] == 4:
    value = value[:,:,0:3]
  elif value.shape[2] == 1:
    # case of depth
    value = np.stack([value, value, value], axis=2).squeeze()
  
  # Clipping for value 0 - 255 int
  if toneMapping:
    value = np.clip(tonemap(value) * 255, 0, 255).astype(np.uint8)
  else:
    value = np.clip(value * 255.0, 0, 255).astype(np.uint8)
    
     
  Image.fromarray(value).save(distpath)
